...
import ando.gallery.adapter.PictureImageGridAdapter;
import ando.gallery.animators.AlphaInAnimationAdapter;
import ando.gallery.animators.AnimationType;
import ando.gallery.animators.SlideInBottomAnimationAdapter;
...
import ando.gallery.manager.UCropManager;
import ando.gallery.decoration.GridSpacingItemDecoration;
import ando.gallery.dialog.PhotoItemSelectedDialog;
...
import ando.gallery.listener.OnItemClickListener;
import ando.gallery.listener.OnPhotoSelectChangedListener;
import ando.gallery.listener.OnQueryDataResultListener;
import ando.gallery.listener.OnRecyclerViewPreloadMoreListener;
import ando.gallery.model.LocalMediaLoader;
import ando.gallery.model.LocalMediaPageLoader;
import ando.gallery.observable.ImagesObservable;
import ando.gallery.permissions.PermissionChecker;
...
import ando.gallery.tools.BitmapUtils;
import ando.gallery.tools.DateUtils;
import ando.gallery.tools.DoubleUtils;
import ando.gallery.tools.JumpUtils;
...
public class PictureSelectorActivity extends PictureBaseActivity implements View.OnClickListener,
    ...
    protected void initWidgets() {
        ...
        mTvPicturePreview.setVisibility(config.enablePreview ? View.VISIBLE : View.GONE);
    ...
    private void AudioDialog(final String path) {
        if (!isFinishing()) {
            ...
            audioDialog.setOnDismissListener(dialog -> {
                ...
                new Handler(Looper.getMainLooper()).postDelayed(() -> stop(path), 30);
                try {
...
//                  }
    ...
    private void manualSaveFolderForPageModel(LocalMedia media) {
        ...
        if (allFolder != null) {
            ...
            // Create All folder
            if (count == 0) {
                allFolder.setName(getString(R.string.picture_camera_roll));
